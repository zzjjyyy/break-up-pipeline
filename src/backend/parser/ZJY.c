...
static List* ZJYExecutor(char* query_string, const char* commandTag, Node* pstmt, PlannedStmt* plannedstmt, CommandDest dest, char* relname, char* completionTag, Query* querytree, Index* transfer_array, List* FKlist, MemoryContext oldcontext);
...
//The interface
void doZJYparse(const char* query_string, const char* commandTag, Node* pstmt, Query* querytree, char* completionTag)
{
	List* FKlist = NIL;
	...
	if (querytree->commandType == CMD_UTILITY)
	{
		...
		ZJYExecutor(query_string, commandTag, pstmt, plannedstmt, DestRemote, NULL, completionTag, querytree, NULL, NIL, oldcontext);
		return;
	}
	ListCell* lc;
	int length = 0;
...
static void Recon(char* query_string, char* commandTag, Node* pstmt, Query* ori_query, char* completionTag)
{
	...
	if (global_query->commandType == CMD_UTILITY)
	{
		...
		ZJYExecutor(query_string, commandTag, pstmt, plannedstmt, DestRemote, NULL, completionTag, NULL, NULL, NIL, oldcontext);
		return;
	}
	...
	while (plannedstmt = ZJYOptimizer(global_query, graph, transfer_array, length))
	{
		...
		FKlist = ZJYExecutor(query_string, commandTag, pstmt, plannedstmt, mydest, relname, completionTag, global_query, transfer_array, FKlist, oldcontext);
		//finish_xact_command();
