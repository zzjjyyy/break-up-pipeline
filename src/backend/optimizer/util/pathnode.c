 ...
 */
/*
* Edited by zhaojy20 in line 2525-2543.
*/
#include "postgres.h"
...
DirectMapPath* create_directmap_path(PlannerInfo* root, RelOptInfo* joinrel, JoinType jointype, JoinCostWorkspace* workspace, JoinPathExtraData* extra, Path* outer_path, Path* inner_path, Relids required_outer, List* restrict_clauses)
{
	DirectMapPath* pathnode = makeNode(DirectMapPath);
	pathnode->jpath.path.pathtype = T_DirectMap;
	pathnode->jpath.path.parent = joinrel;
	pathnode->jpath.path.pathtarget = joinrel->reltarget;
	pathnode->jpath.path.param_info = get_joinrel_parampathinfo(root, joinrel, outer_path, inner_path, extra->sjinfo, required_outer, &restrict_clauses);
	pathnode->jpath.path.parallel_aware = false;
	...
	pathnode->jpath.innerjoinpath = inner_path;
	pathnode->jpath.joinrestrictinfo = restrict_clauses;
	final_cost_directmap(root, pathnode, workspace, extra);
	return pathnode;
}
