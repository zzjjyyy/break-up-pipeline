...
static void hash_inner_and_outer(PlannerInfo *root, RelOptInfo *joinrel,
								 RelOptInfo *outerrel, RelOptInfo *innerrel,
								 JoinType jointype, JoinPathExtraData *extra);
static bool CheckforQualify(RelOptInfo* outerrel, RelOptInfo* innerrel);
static void considerDirectMap(PlannerInfo* root, RelOptInfo* joinrel, RelOptInfo* outerrel, RelOptInfo* innerrel, JoinType jointype, JoinPathExtraData* extra);
...
/*
 * try_directmap_path
 *	  Consider a direct map path; if it appears useful, push it into the joinrel's pathlist via add_path().
 * 
 */
static void try_directmap_path(PlannerInfo* root, RelOptInfo* joinrel, Path* outer_path, Path* inner_path, List* hashclauses, JoinType jointype, JoinPathExtraData* extra)
{
	Relids required_outer;
	JoinCostWorkspace workspace;
	...
	initial_cost_directmap(root, &workspace, jointype, hashclauses, outer_path, inner_path, extra);
	if (add_path_precheck(joinrel, workspace.startup_cost, workspace.total_cost, NIL, required_outer))
	{
		add_path(joinrel, (Path*)create_directmap_path(root, joinrel, jointype, &workspace, extra, outer_path, inner_path, required_outer, extra->restrictlist));
	}
	else
	{
		bms_free(required_outer);
...
extern Index* transfer_array;
extern bool* is_relationship;
static bool CheckforQualify(RelOptInfo* outerrel, RelOptInfo* innerrel)
{
	if (innerrel->relid > 0)
	{
		...
		for (int i = 0;; i++)
		{
			if (transfer_array[i] == innerrel->relid)
			{
				global_no = i;
				break;
			}
...
static void considerDirectMap(PlannerInfo* root, RelOptInfo* joinrel, RelOptInfo* outerrel, RelOptInfo* innerrel, JoinType jointype, JoinPathExtraData* extra)
{
	...
	if (hashclauses)
	{
		...
		if (jointype == JOIN_UNIQUE_OUTER)
		{
			...
			Assert(cheapest_total_outer);
			jointype = JOIN_INNER;
			try_directmap_path(root, joinrel, cheapest_total_outer, cheapest_total_inner, hashclauses, jointype, extra);
		}
		else if (jointype == JOIN_UNIQUE_INNER)
		{
			cheapest_total_inner = (Path*)create_unique_path(root, innerrel, cheapest_total_inner, extra->sjinfo);
			...
			try_directmap_path(root, joinrel, cheapest_total_outer, cheapest_total_inner, hashclauses, jointype, extra);
			if (cheapest_startup_outer != NULL && cheapest_startup_outer != cheapest_total_outer)
				try_directmap_path(root, joinrel, cheapest_startup_outer, cheapest_total_inner, hashclauses, jointype, extra);
		}
		else
		{
			ListCell* lc1;
			...
			foreach(lc1, outerrel->cheapest_parameterized_paths)
			{
				...
				foreach(lc2, innerrel->cheapest_parameterized_paths)
				{
					...
					if (outerpath == cheapest_startup_outer && innerpath == cheapest_total_inner)
						continue;
					try_directmap_path(root, joinrel, outerpath, innerpath, hashclauses, jointype, extra);
				}
			}
		}
	}
