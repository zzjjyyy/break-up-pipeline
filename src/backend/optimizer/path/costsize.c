...
void initial_cost_hashjoin(PlannerInfo *root, JoinCostWorkspace *workspace, JoinType jointype, List *hashclauses, Path *outer_path, Path *inner_path, JoinPathExtraData *extra, bool parallel_hash)
{
	 ...
	 */
	ExecChooseHashTableSize(inner_path_rows_total, inner_path->pathtarget->width, true, parallel_hash, outer_path->parallel_workers, &space_allowed, &numbuckets, &numbatches, &num_skew_mcvs);
	/*
...
/*
 * initial_cost_directmap
 *	  Preliminary estimate of the cost of a directmap join path.
 *
 * This must quickly produce lower-bound estimates of the path's startup and
 * total costs.  If we are unable to eliminate the proposed path from
 * consideration using the lower bounds, final_cost_nestloop will be called
 ...
 * 'extra' contains miscellaneous information about the join
 */
void initial_cost_directmap(PlannerInfo* root, JoinCostWorkspace* workspace, JoinType jointype, List* hashclauses, Path* outer_path, Path* inner_path, JoinPathExtraData* extra)
{
	Cost startup_cost = 0;
	Cost run_cost = 0;
	double outer_path_rows = outer_path->rows;
	...
	run_cost += outer_path->total_cost - outer_path->startup_cost;
	startup_cost += inner_path->total_cost;
	startup_cost += (cpu_operator_cost * 0 + cpu_tuple_cost) * inner_path_rows;
	run_cost += cpu_operator_cost * 0 * outer_path_rows;
	ExecChooseHashTableSize(inner_path_rows_total, inner_path->pathtarget->width, true, false, outer_path->parallel_workers, &space_allowed, &numbuckets, &numbatches, &num_skew_mcvs);
	if (numbatches > 1)
	{
		double outerpages = page_size(outer_path_rows, outer_path->pathtarget->width);
		double innerpages = page_size(inner_path_rows, inner_path->pathtarget->width);
		startup_cost += seq_page_cost * innerpages;
		run_cost += seq_page_cost * (innerpages + outerpages);
	}
	workspace->startup_cost = startup_cost / 2;
	workspace->total_cost = (startup_cost + run_cost) / 2;
	workspace->run_cost = run_cost / 2;
	workspace->numbuckets = numbuckets;
	workspace->numbatches = numbatches;
	workspace->inner_rows_total = inner_path_rows_total;
}
...
/*
 * final_cost_directmap
 *	  Final estimate of the cost and result size of a directmap join path.
 *
 * 'path' is already filled in except for the rows and cost fields
 * 'workspace' is the result from initial_cost_nestloop
 * 'extra' contains miscellaneous information about the join
...
void final_cost_directmap(PlannerInfo* root, DirectMapPath* path, JoinCostWorkspace* workspace, JoinPathExtraData* extra)
{
	...
	else
	{
		innerbucketsize = 1.0;
		innermcvfreq = 1.0;
		foreach(hcl, path->jpath.joinrestrictinfo)
		{
			RestrictInfo* restrictinfo = lfirst_node(RestrictInfo, hcl);
			Selectivity thisbucketsize;
			Selectivity thismcvfreq;
	...
	{
		hashjointuples = approx_tuple_count(root, &path->jpath, path->jpath.joinrestrictinfo);
	}
	startup_cost += qp_qual_cost.startup / 2;
	cpu_per_tuple = cpu_tuple_cost + qp_qual_cost.per_tuple;
	run_cost += cpu_per_tuple * hashjointuples / 2;
	startup_cost += path->jpath.path.pathtarget->cost.startup / 2;
	run_cost += path->jpath.path.pathtarget->cost.per_tuple * path->jpath.path.rows / 2;
	path->jpath.path.startup_cost = startup_cost;
	path->jpath.path.total_cost = startup_cost + run_cost;
	return;
}
