...
#define NEWBETTER 1
#define OLDBETTER 2
#define RelationshipCenter 0
#define EntityCenter 1

//which plan is better?
static int f1(PlannedStmt* new, PlannedStmt* old);
static int f2(PlannedStmt* new, PlannedStmt* old);
static int f3(PlannedStmt* new, PlannedStmt* old);
static int f4(PlannedStmt* new, PlannedStmt* old);
...
static bool* List2Graph(bool* is_relationship, List* joinlist, int length, int strategy);
...
static void spqFK(char* query_string, char* commandTag, Node* pstmt, Query* ori_query, char* completionTag)
{
	...
	bool* graph = List2Graph(is_relationship, Joinlist, length, RelationshipCenter);
...
static PlannedStmt* ZJYOptimizer(Query* global_query, bool* graph, Index* transfer_array, int length)
{
	...
	else
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			else if (f4(candidate_result, result) == NEWBETTER)
			{
				lowest_startup_cost = candidate_result->planTree->startup_cost;
				lowest_total_cost = candidate_result->planTree->total_cost;
				pfree(result);
...
static bool* List2Graph(bool* is_relationship, List* joinlist, int length, int strategy)
{
	...
	foreach(lc1, joinlist)
	{
		...
		//有向graph[x][y], x->y
		if (strategy == RelationshipCenter)
		{
