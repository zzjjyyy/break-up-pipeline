	...
	else
	{
		...
		if (!his->is_true)
		{
			...
			if (outer_rel && inner_rel)
			{
				...
				return clamp_row_est(nrows / 1000);
			}
			/* Base Relation */
			else
			{
...
int learnSelectivity(const QueryDesc* queryDesc, const PlanState* planstate, double* rows)
{
	...
	{
		return learnSelectivity(queryDesc, planstate->lefttree, rows);
	}
	...
	{
		left = learnSelectivity(queryDesc, planstate->lefttree, &left_tuple);
	}
	// Get the true cardinality of the righttree if exists.
	if (planstate->righttree)
	{
		right = learnSelectivity(queryDesc, planstate->righttree, &right_tuple);
	}
	if (plan->type == T_Agg)
	{
		return 0;
