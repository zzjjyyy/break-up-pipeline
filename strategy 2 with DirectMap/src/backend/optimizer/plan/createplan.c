 ...
 */
/*
* Edited by zhaojy20 in line 124, 193, 303, 909-911.
*/
#include "postgres.h"
...
static DirectMap* create_directmap_plan(PlannerInfo* root, DirectMapPath* best_path);
...
static DirectMap* make_directmap(List* tlist, List* joinclauses, List* otherclauses, List* directParams, Plan* lefttree, Plan* righttree, JoinType jointype, bool inner_unique);
...
static MergeJoin *make_mergejoin(List *tlist, List *joinclauses, List *otherclauses, List *mergeclauses,
								 ...
								 Plan *lefttree, Plan *righttree, JoinType jointype, bool inner_unique, bool skip_mark_restore);
...
create_plan_recurse(PlannerInfo *root, Path *best_path, int flags)
{
	...
	switch (best_path->pathtype)
	{
		...
		case T_GatherMerge:
			...
			break;
		default:
			elog(ERROR, "create_plan_recurse: unrecognized node type: %d", (int) best_path->pathtype);
...
static Plan* create_join_plan(PlannerInfo *root, JoinPath *best_path)
{
	...
	switch (best_path->path.pathtype)
	{
		...
		case T_NestLoop:
			...
			break;
		case T_DirectMap:
			plan = (Plan*)create_directmap_plan(root, (DirectMapPath*)best_path);
			break;
		default:
			elog(ERROR, "In create_join_plan: unrecognized node type: %d", (int) best_path->path.pathtype);
			plan = NULL;		/* keep compiler quiet */
			break;
	}
	/*
...
static DirectMap* create_directmap_plan(PlannerInfo* root, DirectMapPath* best_path)
{
	...
	if (IS_OUTER_JOIN(best_path->jpath.jointype))
	{
		extract_actual_join_clauses(joinrestrictclauses, best_path->jpath.path.parent->relids, &joinclauses, &otherclauses);
	}
	else
	{
		joinclauses = extract_actual_clauses(joinrestrictclauses, false);
	...
	join_plan = make_directmap(tlist, joinclauses, otherclauses, directParams, outer_plan, inner_plan, best_path->jpath.jointype, best_path->jpath.inner_unique);
	copy_generic_path_info(&join_plan->join.plan, &best_path->jpath.path);
	return join_plan;
}
...
static DirectMap* make_directmap(List* tlist, List* joinclauses, List* otherclauses, List* directParams, Plan* lefttree, Plan* righttree, JoinType jointype, bool inner_unique)
{
	DirectMap* node = makeNode(DirectMap);
	Plan* plan = &node->join.plan;
	plan->targetlist = tlist;
	plan->qual = otherclauses;
	plan->lefttree = lefttree;
	...
	node->join.inner_unique = inner_unique;
	node->join.joinqual = joinclauses;
	node->directParams = directParams;
	return node;
}
