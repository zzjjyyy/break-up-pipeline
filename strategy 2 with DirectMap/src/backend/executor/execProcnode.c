...
ExecInitNode(Plan *node, EState *estate, int eflags)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_HashJoin:
			...
			break;
		case T_DirectMap:
			result = (PlanState*)ExecInitDirectMap((DirectMap*)node, estate, eflags);
			break;
			/*
		...
		default:
			elog(ERROR, "In ExecInitNode: unrecognized node type: %d", (int) nodeTag(node));
...
ExecEndNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_DirectMapState:
			ExecEndDirectMap((DirectMapState*)node);
			break;
		...
		default:
			elog(ERROR, "In ExecEndNode: unrecognized node type: %d", (int) nodeTag(node));
			break;
	}
}
